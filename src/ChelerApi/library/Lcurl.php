...
class Lcurl
{
    ...
    private $cookie = '/data/cookie.txt';
    // cookie保存路径,相对于APP_PATH
    private $timeout = 10;
     ...
     * @param string $url
     *            请求的URL的完整地址如:http://www.initphp.com/
     * @param array|string $data
     *            post/put/del提交的数据，可以是query string或php数组
     * @param array $options
     *            自定义参数，格式与curl_setopt_array要求一致
     * @return string
     * @throws Exception
     */
    public function exec($url, $data = [], array $options = [])
    {
        $curl = curl_init();
        if ($url)
