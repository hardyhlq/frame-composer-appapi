...
class Lupload
{
    ...
    public function upload($name, $newName, $path, $params = array())
    {
        ...
        if ($result == false) {
            ...
            if ($checkContentResult !== true)
                return $this->error($checkContentResult); // 检测上传错误码
            return $uploadInfo;
        }
    }
     ...
     *
     * @param array $params
     *            array('maxSize' => 文件上传最大,'allowFileType' => 允许上传的文件类型)
     * @return
     *
     */
    ...
    private function checkContent($uploadInfo)
    {
        ...
        } elseif (in_array($uploadInfo['ext'], array(
            'gif',
            'jpg',
            'jpeg',
            'png',
