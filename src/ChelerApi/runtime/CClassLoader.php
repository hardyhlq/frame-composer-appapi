...
class CClassLoader
{
        ...
        if (false !== ($pos = strrpos($class, '\\'))) {
            ...
            foreach (self::$definitions as $ns => $dirs) {
                ...
                foreach ($dirs as $dir) {
                     ...
                     */
                    if (0 === preg_match('#(.+)(if|client|processor|rest)$#i', $class, $n) and 0 === preg_match('#(.+)_[a-z0-9]+_(args|result)$#i', $class, $n)) {
                        $className = 'Types';
                    } else {
                        $className = $n[1];
                    }
                    
                    $file = $dir . DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, $namespace) . DIRECTORY_SEPARATOR . $className . '.php';
    ...
    public static function findProtoBufFile($class)
    {
        ...
        $file = APP_PATH . ChelerApi::getConfig('protobuf.path') . '/' . str_replace('\\', '/', $class) . '.php';
