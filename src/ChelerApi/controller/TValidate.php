...
trait TValidate
{
    ...
    {
        return preg_match('/^((\(\d{2,3}\))|(\d{3}\-))?(10|11|13|14|15|17|18|19)\d{9}$/', trim($value));
    }
    ...
    {
        return preg_match('/^([\xE4-\xE9][\x80-\xBF][\x80-\xBF])+$/u', trim($value));
    }
    ...
    public function is_method($object, $method)
    {
        ...
        return method_exists($object, $method) && is_callable([
            $object,
            $method
        ]);
    }
